...
export function mergeInitMethods<TTarget, TArgs extends any[]>(
  ...
  const m1 =
    method1
    && ((method1 as () => void | PromiseLike<undefined>).bind(target1) as InitMethod<void, TArgs>);
  const m2 =
    method2
    && ((method2 as () => void | PromiseLike<undefined>).bind(target2) as InitMethod<void, TArgs>);
